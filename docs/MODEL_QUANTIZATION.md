...
Run the same validation manually inside an environment with TensorFlow:

```bash
cd app
python extract_model_params.py model/model.tflite --target a8
python extract_model_params.py model/model.tflite --target a9
```
...
- `per-axis quantized tensors; ARTPEC-8 requires per-tensor`: re-export with
   `export_yolov8.py --target a8` or `export_gesture.py --target a8`.

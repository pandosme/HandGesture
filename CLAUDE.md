...
HandGesture is an ACAP application that runs a YOLOv8 hand detector followed by a YOLOv8 gesture classifier directly on Axis cameras with ARTPEC-8 and ARTPEC-9 chipsets. It exports detections through MQTT, ONVIF, and HTTP.
...
1. `build.sh` uses Docker with `axisecp/acap-native-sdk:12.11.0-aarch64-ubuntu24.04`
2. The Dockerfile selects and validates the requested chip-specific detector and gesture models
3. The default build emits separate `artpec8.eap` and `artpec9.eap` packages
...
**app/model/model-a8.tflite / model-a9.tflite**
- Split-output YOLOv8 hand detectors with uint8 I/O
- A8 uses per-tensor quantization; A9 uses per-channel weight quantization

**app/model/gesture-a8.tflite / gesture-a9.tflite**
- YOLOv8 gesture classifiers with uint8 I/O and chip-specific quantization
...
2. **Inference**: `Model_Inference(buffer)` runs the YOLOv8 detector and gesture classifier via larod
...
On camera via SSH: `journalctl -f -u handgesture`
